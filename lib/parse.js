...
let parse = function (opts) {
  let data
  let version = '1.0'
  console.log(util.inspect(opts), opts.swagger.swagger)
  if (_.result(opts, 'swagger.swagger', false)) {
    version = _.result(opts, 'swagger', false)
  } else if (_.result(opts, 'swagger.openapi', false)) {
    version = _.result(opts, 'swagger.openapi', false)
  }
  version = semver.valid(version)
  if (version) {
    console.error('Version of swagger file is not detected')
    return
  }
  if (semver.satisfies(version, '<=1.9.9')) {
    data = getViewForSwagger1(opts)
  } else if (semver.satisfies(version, '>=2.0.0 <3.0.0')) {
    data = getViewForSwagger2(opts)
  } else if (semver.satisfies(version, '>=3.0.0')) {
    data = getViewForSwagger2(opts)
  }
  return data
}
